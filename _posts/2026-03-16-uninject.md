...
v2.1.0에서는 IScope 기반의 Pure C# service layer가 완성되었음. Create<T>()를 통한 생성자 주입, Tickable/Destroyable 라이프사이클, Named bindings, SceneExitPolicy, 풀링 API, 그리고 Dependency Graph와 Bake Validator까지 엔터프라이즈 수준의 진단 체계가 갖추어졌음.  
코어 엔진의 무결성은 이미 v1.1.1에서 검증되었으며, 이번 버전은 그 위에 완전한 Scope/Lifetime 아키텍처를 올린 형태임.
